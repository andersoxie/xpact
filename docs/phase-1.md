...
- Published benchmark table against the available same-machine libexpat baseline in `docs/benchmarks.md`, with raw rows generated by `scripts/run_benchmarks.ps1`.
...
- Direct C libexpat benchmark row once a C compiler is available on the benchmark machine.
